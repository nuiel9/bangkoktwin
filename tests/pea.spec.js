...
test("PEA monitoring, feeder filters, analytics and offline assets work", async ({
  page,
}) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.goto("/");
  await page.waitForFunction(() => window.__peaReady);
  await expect(page).toHaveTitle("PEA · Distribution Digital Twin");
  ...
  await page
    ...
    .click();
  await expect(page.locator("#pea-asset")).toHaveValue("DT-002");
  await page.locator("#pea-feeder").selectOption("PTY-02");
  await expect(page.locator("#p-fleet-rows tr")).toHaveCount(4);
  await expect(page.locator("#p-online")).toHaveText("3/ 4");
  await expect(page.locator("#pea-asset")).toHaveValue("DT-005");
  await page.locator('.p-tabs [data-mode="dtma"]').click();
  await expect(page.locator("#p-analytics")).toBeVisible();
  await expect(page.locator(".p-heat-row")).toHaveCount(4);
  await page.locator("#pea-search").fill("DT-008");
  ...
  await expect(page.locator("#pea-tpo")).toBeDisabled();
  await expect(page.locator(".p-no-data")).toContainText(
    "electrical state unknown",
  );
  await page.locator("#pea-about").click();
  await expect(page.locator("#pea-info")).toBeVisible();
...
test("scenarios conserve active power, improve sample balance and export consistent data", async ({
  ...
  const values = () =>
    page.evaluate(() => {
      const { state, assets, reading } = window.__pea;
      return reading(
        assets.find((a) => a.id === state.selected),
        state,
      );
    });
  await page.locator('.p-tabs [data-mode="tpo"]').click();
  const base = await values();
  await page.locator("#pea-tpo").check();
  ...
  await page.locator("#pea-ev").fill("75");
  expect((await values()).kw).toBeGreaterThan(compensated.kw);
  await page.locator("#pea-time").fill("720");
  const midday = await values();
  await page.locator("#pea-solar").fill("70");
  expect((await values()).kw).toBeLessThan(midday.kw);
  await page.locator("#pea-time").fill("0");
  const night = await values();
  await page.locator("#pea-solar").fill("0");
  expect((await values()).kw).toBeCloseTo(night.kw, 8);
  await page.locator("#pea-asset").selectOption("DT-007");
  await expect(page.locator("#pea-tpo")).not.toBeChecked();
  await page.locator("#pea-time").fill("1080");
  const downloadPromise = page.waitForEvent("download");
  await page.locator("#pea-export").click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toBe("pea-transformers-1800.csv");
  ...
  await page.locator("#pea-scenario-reset").click();
  await expect(page.locator("#pea-ev")).toHaveValue("0");
  await expect(page.locator("#pea-solar")).toHaveValue("0");
  await page.locator("#pea-play").click();
  await expect(page.locator("#pea-clock")).not.toHaveText("18:00");
  await page.locator("#pea-play").click();
});

test("PEA mobile interface fits and keeps controls usable", async ({
  page,
}) => {
  ...
  await page.locator("#pea-feeders").uncheck();
  await page.locator("#pea-asset").selectOption("DT-006");
